...
Размер точки отображает степень схожести страны, а цвет - количество дней отставания. В качестве цветовой гаммы был выбран непрерывный градиент, цвет каждой точки автоматически определялся сервисом в зависимости от отставания. Также были созданы графики новых случаев госпитализации пациентов, больных COVID-19 в выбранной стране на протяжении выбранного периода времени, а также график заболеваемости, смертности и вакцинации.
...
Также мы исследовали другие способы прогнозирования для сравнения полученных предсказаний.
...
Как была построена модель Prophet: в качестве регрессоров берётся история заболеваемости для текущей страны и двух самых похожих стран со сдвигом на количество дней отставания. Все аналогично как и с линейной регрессией, но Prophet для предсказания требуется история заболеваемости из других стран, которая у нас есть (так как мы берём страны, которые отстают от нашей больше, чем на количество дней, которые хотим предсказывать).
