...
def get_cases(days):
    data = httpx.get(
        f"https://storage.yandexcloud.net/covid-19/cases{('_' + str(days)) if days != 0 else ''}.pickle").content
...
def get_predict_linear(country, lags, need_predict, cases_all, use_filter=True, num_features=30):
    ...
    for window in range(num_features, (
            a.index[-1] - threshold).days):  # window проходит значения от кол-ва пр-ков до кол-ва дней в рассм. cases
